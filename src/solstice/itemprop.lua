...
function M.AbilityScore(ability, mod)
   ...
   if mod < 0 then
      eff:SetValues(M.DECREASED_ABILITY_SCORE, ability, 21, math.clamp(-mod, 1, 12))
   else
      eff:SetValues(M.ABILITY_BONUS, ability, 1, math.clamp(mod, 1, 12))
   end
...
function M.AC(value)
   ...
   if value < 0 then
      eff:SetValues(M.DECREASED_AC, -1, 20, math.clamp(-value, 1, 20))
   else
      eff:SetValues(M.AC_BONUS, -1, 2, math.clamp(value, 1, 20))
   end
...
function M.ArcaneSpellFailure(value)
   local eff = M.CreateItempropEffect()
   eff:SetValues(M.ARCANE_SPELL_FAILURE, nil, 27, value)
   return eff
end
...
function M.AttackModifier(value)
   ...
   if value < 0 then
      eff:SetValues(M.DECREASED_ATTACK_MODIFIER, nil, 25, math.clamp(value, 1, 5))
   else
      eff:SetValues(M.ATTACK_BONUS, nil, 20, math.clamp(value, 1, 20))
   end
   return eff
end
...
function M.EnhancementModifier(amount)
   ...
   if amount < 0 then
      eff:SetValues(M.DECREASED_ENHANCEMENT_MODIFIER, nil, 20, math.clamp(-amount, 1, 20))
   else
      eff:SetValues(M.ENHANCEMENT_BONUS, nil, 2, math.clamp(amount, 1, 20))
   end
...
function M.Mighty(value)
   local eff = M.CreateItempropEffect()
   eff:SetValues(M.MIGHTY, nil, 2, math.clamp(value, 1, 20))
   return eff
end
